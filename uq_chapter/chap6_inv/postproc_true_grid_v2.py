...
#%%
# construct prior
x_mean          = np.array([[Rp_orig/Rd_orig],[C_orig],[Rp_orig+Rd_orig]])

def p_prior(x):
    # p_prior = 1
    RpRd_ratio, C, R_tot = x[0][0], x[1][0], x[2][0]
    Rp = RpRd_ratio * R_tot / (1 + RpRd_ratio)
    Rd = R_tot - Rp
...
for i in tqdm(np.arange(qoi_min.shape[0])):
    for j in np.arange(qoi_min.shape[1]):
        for k in np.arange(qoi_min.shape[2]):
            theta = np.array([[Rp_mesh[i,j,k]/Rd_mesh[i,j,k]], [C_mesh[i,j,k]], [Rd_mesh[i,j,k]]+Rp_mesh[i,j,k]])
