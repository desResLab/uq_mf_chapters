import os
import sys
file_path = '/Users/chloe/Documents/Stanford/ME398_Spring/marsden_uq/inv_mc/inv_prelim/test_transformation/circuit_models/'
...
# Inputs: 
#      para:
#         	R: range of proximal resistance
#         	C:  range of capacitance
# Outputs:
#         Y: min(P_p), max(P_p), mean(P_p)
def RC_sim(para, totalCycles, cycleTime):
    ...
    Qp            = np.loadtxt(file_path+'inlet.flow')
	...
	# define ode system, the RC model
    def f(t, state):
        #return R * Qp_interp(t,1) + Qp_interp(t)/C - (state - Pd)/(C * R)
        return Qp_interp(t)/C - (state - Pd)/(C * R)
...
def RCR_sim(para, totalCycles, cycleTime, plot=False):
    ...
    #para                     = para.numpy()
    para                     = np.array(para)
    Rp_star, Rd_star, C_star = para[0], para[1], para[2]
    ...
    if plot:
        ...
        plt.plot(t_eval, P_p.y[0,:]/1333.22)
        plt.show()

    return [y_target.min(), y_target.max(), y_target.mean()]

if __name__ == "__main__":
    print(RC_sim([1000, 5e-5]))